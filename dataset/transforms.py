...
class GroupRandomCrop(object):
    ...
    def __call__(self, img_group):
        ...
        out_images = list()
        h1 = h - th#random.randint(0, max(0, h - th))
...
class GroupRandomCropKeepBottom(object):
    def __init__(self, cropsize):
        if isinstance(cropsize, numbers.Number):
            self.cropsize = (int(cropsize), int(cropsize))
        else:
    ...
    def __call__(self, img_group):
        ...
        kh = np.random.randint(0,th,1)[0]
        kw1 = np.random.randint(0,tw,1)[0]
        kw2 = np.random.randint(0,tw,1)[0]
#        print(kh,kw1,kw2)
        for img in img_group:
            assert (img.shape[0] == h and img.shape[1] == w)
            
            
            out_images.append(img[kh:, kw1:w-kw2, ...])
        return out_images
...
class GroupRandomScale(object):
    ...
    def __call__(self, img_group):
        assert (len(self.interpolation) == len(img_group))
#        scalew = random.uniform(self.size[0], self.size[0])
        scaleh = random.uniform(self.size[1], self.size[1])
        out_images = list()
        for img, interpolation in zip(img_group, self.interpolation):
            out_images.append(cv2.resize(img, None, fx=1.0, fy=scaleh, interpolation=interpolation))
            if len(img.shape) > len(out_images[-1].shape):
                out_images[-1] = out_images[-1][..., np.newaxis]  # single channel image
        return out_images
...
class GroupColorAugment(object):
    def pca_color_augmentation_numpy(self, image_array_input):
        assert len(image_array_input.shape) == 3 and image_array_input.shape[2] == 3
    ...
    def __call__(self, img_group):
        out_images = list()
        for img in img_group:
            img = self.pca_color_augmentation_numpy(img)
            out_images.append(img)
            break
        if(len(img_group) == 2):
            out_images.append(img_group[1])
        return out_images
...
#                print(np.array(m)[np.newaxis, np.newaxis, ...])
...
#            cv2.imshow('label', (out_images[0] * 100).astype(np.uint8))
#            print(np.unique(out_images[0]))
#            cv2.waitKey()
#            cv2.destroyAllWindows()
        return out_images
  
class GroupRandomErase(object):
...
#        print(np.shape(img))
        ...
        target_area = np.random.uniform(self.sl, self.sh, 1) * area
#            print(np.shape(target_area))
